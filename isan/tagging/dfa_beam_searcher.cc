...
public:
    ...
    std::vector< std::map<KEY,State_Info> > sequence;
    ...
    void thrink(int step,std::vector<std::pair<KEY,SCORE> >& top_n){
        ...
        std::map<KEY,State_Info>* map=&(this->sequence[step]);
        typename std::map<KEY,State_Info>::iterator it;
    ...
    std::vector<ACTION> call(KEY init_key,int steps){
        ...
        this->sequence.clear();
        this->sequence.push_back(std::map<KEY,State_Info>());
